...
import scrapy
import re
from vip_spider.items import RealTimeDangqiItem
import datetime
import time
import json
import web
# http://compass.vis.vip.com/newRealTime/situation/distribution?callback=jQuery33101217815476260572_1523505238223&brandStoreName=Bata&_=1523505238236
...
class RealTime_Dangqi_Spider(scrapy.Spider):
    ...
    def start_requests(self):
        for brand in brand_list:
            url = 'http://compass.vis.vip.com/newRealTime/situation/distribution?callback=jQuery33101217815476260572_1523505238223&brandStoreName={}&_={}'.format(
                brand, int(time.time() * 1000))
    ...
    def real_parse(self, response):
        ...
        # items = RealTimeDangqiItem()
        # RealTime_Dangqi_Spider.brand_num += 1
        content = re.findall('jQuery33101217815476260572_\d+\(([\s\S]*?)\);', content)[0]
        if '"code":"0"' not in content:
            ...
            if brand == '真美诗Joy%26Peace':
                brand = '真美诗Joy&Peace'
            # items['brand'] = brand
            sales_list = con_json['singleResult']['data']['sales'][0]
            uv_list = con_json['singleResult']['data']['uv'][0]
            consumerCount_list = con_json['singleResult']['data']['consumerCount'][0]
            conversion_list = con_json['singleResult']['data']['conversion'][0]
            if sales_list:
                total_item = []
                #RealTime_Dangqi_Spider.brand_num += 1
                for sales in sales_list:
                    items = RealTimeDangqiItem()
                    items['brand'] = brand
                    items['the_type'] = 'sales'
                    items['the_name'] = sales['name']
                    items['weight'] = sales['weight']
                    items['dt'] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
                    total_item.append(items)
                for uv in uv_list:
                    items = RealTimeDangqiItem()
                    items['brand'] = brand
                    items['the_type'] = 'uv'
                    items['the_name'] = uv['name']
                    items['weight'] = uv['weight']
                    items['dt'] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
                    total_item.append(items)
                for consumerCount in consumerCount_list:
                    items = RealTimeDangqiItem()
                    items['brand'] = brand
                    items['the_type'] = 'consumerCount'
                    items['the_name'] = consumerCount['name']
                    items['weight'] = consumerCount['weight']
                    items['dt'] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
                    total_item.append(items)
                for conversion in conversion_list:
                    items = RealTimeDangqiItem()
                    items['brand'] = brand
                    items['the_type'] = 'conversion'
                    items['the_name'] = conversion['name']
                    items['weight'] = conversion['weight']
                    items['dt'] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
                    total_item.append(items)
                #if RealTime_Dangqi_Spider.brand_num == 13:
                #if brand =='Istbelle':
                ...
                url = 'http://compass.vis.vip.com/newRealTime/situation/distribution?callback=jQuery33101217815476260572_1523505238223&brandStoreName={}&_={}'.format(
                    brand, int(time.time() * 1000))
                yield scrapy.Request(url, cookies=cookie, callback=self.real_parse, dont_filter=True,
                                     meta={'brand': brand})
        else:
            url = 'http://compass.vis.vip.com/newRealTime/situation/distribution?callback=jQuery33101217815476260572_1523505238223&brandStoreName={}&_={}'.format(
                brand, int(time.time() * 1000))
