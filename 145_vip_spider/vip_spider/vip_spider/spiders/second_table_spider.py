...
class Second_Table_Spider(scrapy.Spider):

    name = 'second_table_spider'
    sum_uv = 0
    brand_num = 0
    total_item = []
    ...
    def second_parse(self, response):
        ...
        Second_Table_Spider.brand_num += 1
        if '"code":"0"' not in content:
            ...
            for i in range(len(groups_list)):
                flow = flow_list[i]
                group = groups_list[i]
                Second_Table_Spider.sum_uv = Second_Table_Spider.sum_uv + float(flow[-1]['uv'])
                for item in flow:
                    ...
                    items['group_'] = group
                    items['brand'] = brand
                    Second_Table_Spider.total_item.append(items)
        if Second_Table_Spider.brand_num == 13 and Second_Table_Spider.sum_uv != 0:
            for item in Second_Table_Spider.total_item:
                yield item
