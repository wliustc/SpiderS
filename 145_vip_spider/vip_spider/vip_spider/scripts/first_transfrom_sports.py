...
for line in sys.stdin:
    try:
        ...
        kv_str = ','.join(
            ...
            for (k, v)
            in items.items())
        sql = "INSERT INTO t_spider_vip_whole_index_sport(%s) VALUES(%s)" % (key_str, value_str)
...
db.query('insert into t_spider_vip_sign(dt,sign,spider_name,datetime1) value ("{}","1","{}","{}");'.format(now, 'first_table_sports_spider', the_time))
