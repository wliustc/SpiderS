...
for line in sys.stdin:
    try:
        ...
        kv_str = ','.join(
            ...
            for (k, v)
            in items.items())
        sql = "INSERT INTO t_spider_vip_avgOrder_sport(%s) VALUES(%s)" % (key_str, value_str)
        sql = "%s ON DUPLICATE KEY UPDATE %s" % (sql, kv_str)
        db.query(sql)
    except:
        pass
now = datetime.datetime.now().strftime('%Y-%m-%d')
db.query('insert into t_spider_vip_sign(dt,sign,spider_name) value ("{}","1","{}");'.format(now, 'third_table_sports_spider'))