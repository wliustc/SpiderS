...
for line in sys.stdin:
    try:
        ...
        kv_str = ','.join(
            ...
            for (k, v)
            in items.items())
        sql = "INSERT INTO t_spider_vip_whole_index(%s) VALUES(%s)" % (key_str, value_str)
        sql = "%s ON DUPLICATE KEY UPDATE %s" % (sql, kv_str)
        db.query(sql)
        # now = datetime.datetime.now().strftime('%Y-%m-%d')
        # db.query('insert into t_spider_vip_sign(dt,sign,spider_name) value ("{}","1","{}");'.format(now, 'first_table_spider'))
    except:
        pass
the_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
db.query('insert into t_spider_vip_sign(dt,sign,spider_name,datetime1) value ("{}","1","{}","{}");'.format(now, 'first_table_spider', the_time))
