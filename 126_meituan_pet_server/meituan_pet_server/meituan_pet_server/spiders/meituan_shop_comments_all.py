...
class MeituanShopCommentsAllSpider(scrapy.Spider):
    name = "meituan_shop_comments_all"
    allowed_domains = ["m.meituan.com"]
    start_urls = ['http://m.meituan.com/']
    ...
    def get_city_id(self, response):
        ...
        cur = conn.cursor()
        cur.execute("SELECT distinct `mtshop_id`,`host`,`mtshop_name` FROM tuangou_meituan_shop_info where TO_DAYS(NOW()) - TO_DAYS(`dt`) <= 3;")
        tmps = cur.fetchall()
        for tmp in tmps:
            ...
            yield scrapy.Request(url, headers=header, dont_filter=True,
                                 meta={'item': {'shop_id': tmp['mtshop_id'], 'page_name': 1}})

    def parse(self, response):
        datas = response.css('.feedbackCard')
        if not datas and response.meta['item']['page_name']>1:
            self.logger.error('url:%s			data is null', response.request.url)
        for data in datas:
            ...
            item['hash'] = hash(
                data.css('.username::text').extract()[0] + data.css('weak.time::text').extract()[0] + ''.join(
                    data.xpath('./div[@class="comment"]//text()').extract()))
            yield item
        if len(datas) < 15:
            pass
        else:
            ...
            yield scrapy.Request(url, headers=header, dont_filter=True,
                                 meta={'item': {'shop_id': response.meta['item']['shop_id'],
                                                'page_name': page_num}})
