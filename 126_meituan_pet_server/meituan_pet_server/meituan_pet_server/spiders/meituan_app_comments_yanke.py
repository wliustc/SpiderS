...
class MeituanAppCommentsYankeSpider(scrapy.Spider):
    name = "meituan_app_comments_yanke"
    allowed_domains = ["i.meituam.com"]
    start_urls = ['http://i.meituam.com/']
    ...
    def get_city_id(self, response):
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT distinct shop_id FROM o2o.t_hh_meituan_shop_info_tmp;")
        tmps = cur.fetchall()
        for tmp in tmps:
            base_url = 'https://i.meituan.com/poi/%s/feedbacks/page_%s'
            url = base_url % (tmp['shop_id'], 1)
            header = {
                'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8',
                'Accept-Encoding': 'gzip, deflate, br',
                'Accept-Language': 'zh-CN,zh;q=0.8',
            ...
            yield scrapy.Request(url, headers=header, dont_filter=True,
                                 meta={'item': {'shop_id': tmp['shop_id'], 'page_name': 1}})

    def parse(self, response):
        datas = response.css('.feedbackCard')
        for data in datas:
            item = MeituanPetAppYankecommentItem()
