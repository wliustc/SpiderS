...
cursor = conn.cursor()
conn1 = MySQLdb.connect(host="10.15.1.24", port=3306, user="writer", passwd="hh$writer", db="hillinsight",
                        charset="utf8")
...
for line in sys.stdin:
    ...
    table, ','.join(['`%s`' % col for col in cols]), ','.join(['%s' for i in range(len(cols))]))
    print sql
    try:
