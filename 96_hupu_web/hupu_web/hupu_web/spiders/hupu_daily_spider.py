...
class Hupu_Spider(scrapy.Spider):
    ...
    def boards_parse(self, response):
        ...
        items = HupuWebItem()
        pattern = re.compile('<div class="titlelink box"([\s\S]*?)</li>')
        con_list = re.findall(pattern, content)
        if con_list and page <= 30:
            page += 1
            for con in con_list:
                pattern1 = re.compile('class="ansour box">(\d+|-\d+)&nbsp;/&nbsp;(\d+)<')
                num_data = re.findall(pattern1, con)[0]
                reply_num = num_data[0]
                browse_num = num_data[1]
                pattern2 = re.compile('href="/(\d*?)\.html"[\s\S]*?>([\s\S]*?)</a>')
                ...
                if reply_num != '0':
                    pattern3 = re.compile('<div class="endreply box">[\s\S]*?<.*?>(.*?)</a>')
