...
import scrapy
import re
from hospital_info.items import HospitalInfoItem

headers = {"User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_2) AppleWebKit/537.36 (KHTML, like Gecko) "
                         "Chrome/54.0.2840.98 Safari/537.36"}


class Hospital_Info_Spider(scrapy.Spider):

    name = 'hospital_info_spider'
    ...
    def list2parse(self, response):
        ...
        for level in level_list:
            ...
            level_name = level[1]
            # items['level'] = level_name

            yield scrapy.Request(level_link, headers=headers, callback=self.infoparse,
                                 meta={'province': province, 'city_name': city_name,
                                       'level_name': level_name}, dont_filter=True)
