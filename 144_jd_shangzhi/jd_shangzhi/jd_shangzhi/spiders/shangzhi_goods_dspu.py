...
class ShangzhiGoodsDspuSpider(scrapy.Spider):
    name = "shangzhi_goods_dspu"
    ...
    def scan_need_request(self):
        if replenish_history == 1:
            ...
            self.time_parse(1), 't_spider_jdweizhi_goods_details_spu')
            ...
            if temp:
                ...
                db.query('insert into hillinsight.t_spider_jdweizhi_spider(`id`,`dt`,`spider_name`,'
                         '`flag`) VALUE (NULL,"%s","%s","0")' % (self.time_parse(1), 't_spider_jdweizhi_goods_details_spu'))
                return True
