...
from DianPingTuan.items import PetCommentsItem
...
#           'JSESSIONID':'%s'% __md5sum("%s" % time.time()),
#           's_ViewType':'10',
...
#     'cy': '18;',
#     'cye': 'shenyang;',
#     '__utma': '1.306596959.1494215713.1494401139.1494815058.3;',
#     '__utmz': '1.1494215713.1.1.utmcsr=(direct)|utmccn=(direct)|utmcmd=(none);',
#     'JSESSIONID': '8E2B8A4FC39761B8D56EBAD58EC07B9C;',
#     '__utmc': '1;',
#     's_ViewType': '10;',
#     'aburl': '1;',
#     'PHOENIX_ID': '0a0102f1-15c114151d0-436b141;',
#     '__mta': '255702583.1494938118288.1494938121664.1494938229070.3'
# }


# cookie = "_hc.v=9c20f3b2-274d-8559-c306-1785c4b96ebc.%s;" % (int(time.time()))
# cookie = "%sPHOENIX_ID=0a01075d-1561feb7e37-1dcbeb2;JSESSIONID=%s" % (cookie, self.__md5sum("%s" % time.time()))
...
class DianpingtuanSpider(scrapy.Spider):
    ...
    def start_requests(self):
        data = db.query(
            'SELECT distinct dianping_id FROM hospital_base_information WHERE brand_id = 3 and dianping_id is not null and dianping_id!=0;')
        for d in data:
            dianping_id = d.dianping_id
            ...
            #               'PHOENIX_ID': '0a0102f1-15c114151d0-436b141'
            #               }
            # yield Request('http://www.dianping.com/shop/4666482/review_more?pageno=2', errback=self.parse_failure,
            #               callback=self.parse, headers=header, cookies=Cookie)
            ...
            if detail_list:
                for detail in detail_list:
                    ...
                    total_score = ''.join(detail.xpath(
                        './div[@class="content"]/div[@class="user-info"]/span[0]/@class').extract()).strip().replace(
                        '\n',
                        '')
