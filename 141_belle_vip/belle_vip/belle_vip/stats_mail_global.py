...
def get_task_description(tid):
    sql = '''select description from t_platform_task where id=%s'''%tid
    res = db.query(sql)
    if not res:
        return ""
...
class StatsMailer(object):
    ...
    def spider_closed(self, spider):
        ...
        if not spider_stats.has_key('item_scraped_count'):
            cmp_st = "Failed!!! scrapy None"
            com_result = ""
        else:
        ...
        desc = get_task_description(self.task_id)
        print "email-desc :%s"%desc
        mail_sub = "【%s】 Scrapy stats for: %s %s" % (cmp_st,spider.name, desc)
