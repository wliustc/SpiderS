...
class MeituanyakeSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for i in db.query(sql):
            ...
            yield scrapy.Request(url,headers=headers,meta={'url':url,'city_name':city_name,'city_jc':city_jc,'province':province,'headers':headers},dont_filter=True)
    def parse(self, response):
        if response.status != 200 and response.status != 503:
            yield scrapy.Request(response.meta['url'],headers=response.meta['headers'],callback=self.parse, meta=response.meta, dont_filter=True)
        else:
            html = response.body
                ...
                for i in zip(chaifen.div_list(id_s, ci), chaifen.div_list(data_lists, ci)):
                    ...
                    while True:
                        if html.status_code == 200:
                            yield self.parse1(html.text,city_name,province)
                            break
                        else:
                            html = requests.post(url, data=data, headers=headers)
            else:
    ...
    def parse1(self,html,city_name,province):
        itme ={}
        htmls = html.replace('\/','/')
        import json
    ...
    def parse2(self,response):
        if response.status != 200:
            yield scrapy.Request(response.meta['url'],headers=headers, callback=self.parse, meta=response.meta, dont_filter=True)
        else:
            html = response.body
