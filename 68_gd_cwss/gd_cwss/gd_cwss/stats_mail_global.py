...
def warn_condition(pre, cur):
    if pre > cur and 1.0 * (pre - cur) / pre > 0.2:
        return True
    else:
        return False
...
def get_compare_result(tid):
    c_result = ''
    sql = 'SELECT uid, stats_dump from t_platform_jobs where tid=%s and stats_dump is not NULL ORDER BY id desc limit 2;' % tid
    data = db.query(sql)
    if data and len(data) == 2:
        try:
            cur_data = data[0]
            cur = json.loads(cur_data['stats_dump'].replace("'", '"'))
            cur_uid = cur_data['uid']
            pre = json.loads(data[1]['stats_dump'].replace("'", '"'))
            if int(cur['item_scraped_count']) == 0:
                return "Falied scraped None", c_result
            if int(pre['item_scraped_count']) == 0:
                compare_rate = "--"
            else:
                compare_rate = round((float(pre['item_scraped_count']-cur['item_scraped_count'])/pre['item_scraped_count'])*100,2)
                compare_rate = str(compare_rate) + "%"
            url = "http://spider.in.hillinsight.com/manager/job/job_detail/%s"%cur_uid
