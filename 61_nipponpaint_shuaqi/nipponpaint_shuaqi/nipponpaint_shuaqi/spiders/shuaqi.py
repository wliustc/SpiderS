...
from nipponpaint_shuaqi.items import NipponItem
...
class ShuaqiSpider(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs):
        super(ShuaqiSpider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        return [ scrapy.http.Request(
            url = "http://shuaxinfuwu.nipponpaint.com.cn/api/GetPersonHandler.ashx?d=" + urllib.quote(datetime.datetime.now().strftime("%a %b %d %Y %H:%M:%S GMT+0800 ")+'(中国标准时间)'),
            method = "POST",
            headers = {
    ...
    def parse(self, response):
        ...
        for page_num in range(1, total_page_num+1):
            yield scrapy.http.Request(
                url="http://shuaxinfuwu.nipponpaint.com.cn/api/GetPersonHandler.ashx?d=" + urllib.quote(
