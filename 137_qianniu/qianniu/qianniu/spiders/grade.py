...
class GradeSpider(scrapy.Spider):
    ...
    def __init__(self, select_date='yesterday', crawl_brand='all', *args, **kwargs):
        super(GradeSpider, self).__init__(*args, **kwargs)
    ...
    def start_requests(self):
        ...
        if data:
            for brand, val in data.items():
                ...
                cate = '评价概况'
                url = 'https://sycm.taobao.com/qos/review/overview/guide.json?dateType=day&dateRange=%s|%s' %month
    ...
    def parse(self, response):
        try:
            ...
            if str(code) == '0':
                ...
                #                     # print url_xg
                #                     yield Request(url_xg, callback=self.parse, headers=header,
