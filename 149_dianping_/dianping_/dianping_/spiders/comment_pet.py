...
class CommentNotLoginSpider(scrapy.Spider):
    name = "CommentNotLogin"
    ...
    def parse(self, response):
        ...
        sql = "select `dianping_id` as `shop_id` from hillinsight.pet_tuangou_hot_shop where `dianping_id` is not NULL;"
        shop_list = []
    ...
    def re_get_cookie(self, response):
        ...
        for i in range(1,5):
            try:
                ...
                header['User-Agent'] = self.user_agent
                yield scrapy.Request(response.request.url, headers=header, cookies=copy.deepcopy(self.cookies),
                                     callback=self.get_item, dont_filter=True,
                                     meta={'url': response.request.url, 'shop_id': response.meta['shop_id'],
                                           'shop_data': shop_data,
